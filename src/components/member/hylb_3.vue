<template>
  <div class="container">
    <table class="table">
      <tbody>
        <tr>
          <td class="first" style="border: none;">{{$L.t('search')}}:</td>
          <td>
            <div class="userList-search">
              {{$L.t('chessGame.zh3')}}
              <select v-model="selectKey" @change="headSelect(selectKey)">
                <option value="username">{{$L.t('chessGame.zh26')}}</option>
                <option value="nickname">{{$L.t('table.nikename')}}</option>
                <option value="parent">{{$L.t('member.info30')}}</option>
                <option value="AgentUsername">{{$L.t('annoucement.wjtz_3.zh13')}}</option>
                <option value="usertype">{{$L.t('agent.agent_0.memberType')}}</option>
                <option value="timeSlot">{{$L.t('agent.agent_7.jqwt')}}</option>
                <option value="name">{{$L.t('table.name')}}</option>
                <option value="onlinestatus">{{$L.t('chatRoom.zh100')}}</option>
                <option value="accessCount">{{$L.t('member.hylb_3.info1')}}</option>

                <option value="betsCode">{{$L.t('member.info31')}}</option>
                <option value="coin">{{$L.t('member.info32')}}</option>
                <option value="loseMax">{{$L.t('member.info31')}}</option>
                <option value="loseMin">{{$L.t('member.info33')}}</option>
                ...
                <option value="userlevelmin">{{$L.t('member.info35')}}</option>
                <option value="userlevelmax">{{$L.t('member.info36')}}</option>
                <option value="scoreMax">{{$L.t('member.info37')}}</option>
                <option value="scoreMin">{{$L.t('member.info38')}}</option>
                <option value="rechargeMax">{{$L.t('member.info39')}}</option>
                <option value="rechargeMin">{{$L.t('member.info40')}}</option>

                <!-- <option value="borrowMoneyMin">借款金额小于</option>
                <option value="borrowMoneyMax">借款金额大于</option> -->




                <option value="ip">{{$L.t('member.hylb_3.info2')}}</option>
                <option value="account">{{$L.t('member.info41')}}</option>
                <option value="countname">{{$L.t('member.info42')}}</option>
                <option value="attach">{{$L.t('table.remark')}}</option>
              </select>
              ::
              <select v-model="selectInput" v-if="selectKey == 'usertype'">
                <option value="">{{$L.t('agent.agent_8.zh49')}}</option>
                <option value="1">{{$L.t('agent.agent_0.remark')}}</option>
                <option value="2">{{$L.t('agent.agent_0.dl')}}</option>
                <option value="3">{{$L.t('agent.agent_7.csh')}}</option>
              </select>
              <input v-if="selectKey != 'usertype' && selectKey != 'timeSlot' && selectKey != 'onlinestatus' && selectKey != 'accessCount' " type="text" style="display: inline-block;width: 150px" v-model="selectInput">
              <!-- <span>近期未投</span>
              <select v-model="sendParmas.data.timeSlot">
                <option value="">请选择时间</option>
                <option value="1">一周未投</option>
                <option value="2">半月未投</option>
                <option value="3">一个月以上未投</option>
                <option value="4">半年未投</option>
              </select> -->
              <select v-model="selectInput" v-if="selectKey == 'timeSlot'">
                <option value="">{{$L.t('member.info43')}}</option>
                <option value="1">{{$L.t('agent.agent_7.yzwt')}}</option>
                <option value="2">{{$L.t('agent.agent_7.bywt')}}</option>
                <option value="3">{{$L.t('member.info45')}}</option>
                <option value="4">{{$L.t('agent.agent_7.bnwt')}}</option>
              </select>
              <!-- 是否在线 -->
              <select v-model="selectInput" v-if="selectKey == 'onlinestatus'">
                <option value="">{{$L.t('agent.agent_2.all')}}</option>
                <option value="1">{{$L.t('agent.agent_8.zh36')}}</option>
                <option value="2">{{$L.t('member.hylb_3.info3')}}</option>
              </select>
              <span>{{$L.t('agent.agent_7.userType')}}</span>
              <select v-model="sendParmas.data.rechargeMember" >
                <option value="">{{$L.t('agent.agent_8.zh49')}}</option>
                <option value="1">{{$L.t('member.info46')}}</option>
                <option value="2">{{$L.t('member.info47')}}</option>
                <option value="3">{{$L.t('member.info48')}}</option>
                <option value="4">{{$L.t('member.info49')}}</option>
                <option value="5">{{$L.t('member.info50')}}</option>
              </select>
              <div class="submitSeach">
                {{$L.t('member.info51')}}
                <select v-model="rechargeLimit">
                  <option value="=">=</option>
                  <option value=">">></option>
                  <option value="<"><</option>
                  <option value=">=">>=</option>
                ...
                <input style="width:80px" type="number" v-model="rechargeNumber">
              </div>
              <span>
                <span v-if="selectKey == 'accessCount'">{{$L.t('member.hylbCopy.info1')}}</span><span v-else>{{$L.t('member.hylbCopy.info2')}}</span>{{$L.t('table.time')}}</span>
              <timer v-if="$route.query.id && $route.query.id == 'today'" @sendTime="getSearchTimer"  :day="diyDays" :paramTime="showLogData"></timer>
              <timer v-if="!$route.query.id && $route.query.id != 'today'" @sendTime="getSearchTimer" :day="diyDays"></timer>
              <span></span>

              <div class="ser_list">
                <span>{{$L.t('member.info52')}}:</span>
                <select v-model="sendParmas.data.exclass_id">
                  <option  value="">
                    {{$L.t('agent.agent_8.zh49')}}
                  </option>
              ...
              <div>
                {{$L.t('lottery.zh91')}}
                <select v-model="sendParmas.data.sort" >
                  <option value="">{{$L.t('member.info145')}}</option>
                  <option value="asc">{{$L.t('member.hylb_3.info4')}}</option>
                  <option value="desc">{{$L.t('member.hylb_3.info5')}}</option>
                </select>

                <select v-model="sendParmas.data.order" >
                  <option value="">{{$L.t('member.info145')}}</option>
                  <option value="coin">{{$L.t('table.yue')}}</option>
                  <option value="regTime">{{$L.t('table.registerTime')}}</option>
                </select>
              </div>
              ...
              <div>
                {{$L.t('member.info54')}}:
                <select v-model="sendParmas.data.isEnjoy" >
                  <option value="">{{$L.t('agent.agent_8.zh49')}}</option>
                  <option value="1">{{$L.t('member.info55')}}</option>
                  <option value="0">{{$L.t('member.info56')}}</option>
                </select>
              </div>
          ...
          <td class="right">
            ...
            <button @click="openGrouping()">{{$L.t('member.info59')}}</button>
    ...
    <table class="table" id="out-table">
      <tbody>
        <tr class="head">
          ...
          <td>
            <div style="width: 70px;white-space: normal;">
              {{$L.t('table.sjYK')}}
              <div class="help-tip">
                <p>{{$L.t('member.hylb_3.info6')}}</p>
              </div>
            </div>
          </td>
          <td>{{$L.t('chatRoom.zh83')}}</td>
          <td>{{$L.t('member.info64')}}</td>
          <td>{{$L.t('member.info65')}}</td>
          ...
          <td style="width: 200px;">{{$L.t('table.remark')}}</td>
          <td>{{$L.t('member.info69')}}</td>
        ...
        <tr class="odd" v-for="(item, index) in memberStore.userList.data.data">
          ...
          <td v-if="item.enable == 0">
            ...
            <button class="popBtn" @click="enableChange(item, 1)">{{$L.t('lottery.zh126')}}</button>
          </td>
          <td v-if="item.enable == 1">
            {{$L.t('bonusGame.NiuNiuBetManage.zh42')}}<br/>
            <button  class="popBtn" @click="enableChange(item, 0)">{{$L.t('member.info70')}}</button>
          </td>
          <td>
          ...
          <td  style="white-space: normal;max-width: 200px;word-wrap: break-word;">{{item.attach}}</td>
          <td>{{item.exclass}}</td>
          <td>
            <span>
            ...
            <span>
              <button class="popBtn" @click="bankClick(item)">{{$L.t('member.info41')}}</button>
            </span>
            <br>
            <span>
        ...
        <tr v-if="getPageData.sum == 0">
          <td colspan="21" style="text-align: center">{{$L.t('noData')}}</td>
        </tr>
        <tr>
          <td colspan="21">
        ...
        <div v-if="userVisible" class="form_container">
          ...
          <ul>
            ...
            <li>
              ...
              <div class="formLi_content">
                <select v-model="userForm.exclass_id">
                  <option v-for="(item,index) in memberStore.claslist.data.data" :key="index" :value="item.id">
                    {{item.name}}
                  </option>
                </select>
            ...
            <li v-if="userForm.type == 1">
              ...
              <div class="formLi_content">
                <input type="text" :placeholder="$L.t('member.info182')" v-model="userForm.rCode">
              </div>
              <div class="formLi_hint">
                *
              </div>
        ...
        <div v-if="editInfoVisible" class="form_container">
          ...
          <ul>
            ...
            <!--<li>-->
              ...
              <!--</div>-->
              <!--<div class="formLi_content">-->
                <!--<input type="text" v-model="editInfoVal.needBetsCode" placeholder="请输入打码量">-->
              <!--</div>-->
              <!--<div class="formLi_hint">-->
                <!--*-->
              <!--</div>-->
      ...
      <el-dialog
        ...
        <div v-if="dmVisible" class="table_container">
          <div style="padding: 4px 0  10px 4px;">
            {{$L.t('member.info66')}}:
            <input type="text" :placeholder="$L.t('member.info83')" v-model="editInfoVal.needBetsCode" style="height: 28px;">

            <button @click="updateDML()">{{$L.t('member.info84')}}</button>

            <button @click="checkeDmDetail()" style="margin-left: 10px;">{{$L.t('member.hylb_3.info20')}}</button>
          </div>
          <ul>
      ...
      <!--总打码量-->
      <el-dialog
            :title="$L.t('member.hylb_3.info21')"
            :visible.sync="allDmVisible"
            :modal-append-to-body="allDmVisible"
            :append-to-body="allDmVisible"
            :modal="allDmVisible"
            width="800px">
      <div v-if="allDmVisible" class="model_table table_550" ref="dmDOm">
        <div>
          <p v-if="allDmData.message" style="color: red;">{{allDmData.message}}</p>
          <p v-if="allDmData.cash_time">{{$L.t('member.hylb_3.info22')}} <span style="color: red;">{{allDmData.cash_time}}</span> {{$L.t('member.hylb_3.info23')}}</p>
          <p v-if="allDmData.cashMinAmount">{{$L.t('member.hylb_3.info24')}} : <span style="color: red;">{{allDmData.cashMinAmount}} %</span> </p>
        </div>
        <table class="table">
          <tbody>
          <tr class="head" ref="tabHead">
            <td style="width: 200px;">{{$L.t('common.xm')}}</td>
            <td style="width: 100px;">{{$L.t('member.info124')}}</td>
            <td style="width: 100px;">{{$L.t('member.hylb_3.info25')}}</td>
            <td style="width: 100px;">{{$L.t('member.hylb_3.info26')}}</td>
            <td style="width: 100px;">{{$L.t('member.hylb_3.info27')}}</td>
            <td style="width: 200px;">{{$L.t('table.time')}}</td>
          </tr>
          <tr class="odd" v-for="(item, index) in allDmData.data" :key="index" v-if="allDmData.data.length != 0">
               <td style="max-width: 150px;width: 200px;" v-if="item.liqType != 0" >{{item.info }}</td>
               <td v-if="item.liqType != 0"  style="width: 100px;">{{item.coin }}</td>
               <td v-if="item.liqType != 0" style="width: 100px;">{{item.betsCode}}</td>
               <td v-if="item.liqType != 0" style="width: 100px;">{{item.needBetsCode}}</td>
               <td v-if="item.liqType != 0" style="width: 100px;">{{item.residualBetsCode}}</td>
               <td v-if="item.liqType != 0" style="width: 200px;">{{item.actionTime }}</td>

               <td colspan="6" v-else style="color: red;text-align: center;">{{item.info}}</td>
          </tr>
          <!--<tr v-for="(item, index) in allDmData.data" :key="index" v-if="allDmData.data.length != 0 && item.liqType == 0">-->
            <!--<td colspan="6">{{item.info}}</td>-->
          <!--</tr>-->

          <tr class="info_none" v-if="allDmData.data.length == 0">
            <td colspan="8">
              {{$L.t('noData')}}
            </td>
          </tr>
        ...
        <div v-if="sdckVisible" class="form_container">
          <ul>
            ...
            <li>
              ...
              <div class="formLi_content">
                <input type="number" v-model="formData.payPrice" :placeholder="$L.t('member.info125')">
              </div>
              <div class="formLi_hint">
                {{$L.t('member.info185')}}
              </div>
        ...
        <div v-if="zsljVisible" class="form_container">
          <ul>
            ...
            <li>
              ...
              <div class="formLi_content">
                <input type="text" v-model="formData.remark" :placeholder="$L.t('manager.info16')">
              </div>
              <div class="formLi_hint">
                *
              </div>
          ...
          <table class="table">
            <tbody>
              ...
              <tr class="odd" v-for="(item, index) in memberStore.userBankList.data.data" :key="index">
                ...
                <td>{{times(item.bdtime)}}</td>
                <td>{{times(item.xgtime)}}</td>
                <td>
    ...
    <!-- 批量调整分组 -->
    <el-dialog
            :title="$L.t('member.info59')"
            :visible.sync="groupingVisible"
            :modal-append-to-body="groupingVisible"
            :append-to-body="groupingVisible"
            :modal="groupingVisible"
            width="600px">
      <div v-if="groupingVisible" class="form_container">
        <div class="formBox_title">
          {{$L.t('member.info90')}}
        </div>
        <ul>
          <li>
            ...
            <div class="formLi_content">
              <select v-model="formData.groupId">
                <option value="">{{$L.t('member.info92')}}</option>
                <option
                        v-for="(item, index) in memberStore.claslist.data.data"
          ...
          <li class="formLi_flexCenter">
            <button class="form_commit" @click="submitGrouping()">{{$L.t('sure')}}</button>
            {{$L.t('member.info93')}}{{activeNum}}{{$L.t('agent.agent_0.huiy')}} ({{$L.t('member.info95')}})
          </li>
        </ul>
...
<script>
  ...
  export default {
    ...
    data () {
      return {
        ...
        exportTypeList: [
          ...
          {id: 18, title:  this.$L.t('member.info158'), name:'attach', active: true},
        ],
        chooseData:[],
        showLogData:{
          fromTime:'',
    ...
    created (){
      if(this.$route.query.id && this.$route.query.id == 'today'){
        this.sendParmas.data.fromTime =  this.timeTool(new Date()) + ' 00:00:00'
        this.sendParmas.data.toTime = this.timeTool(new Date()) + ' 23:59:59'
        this.showLogData.fromTime = this.timeTool(new Date()) + ' 00:00:00'
       this.showLogData.toTime = this.timeTool(new Date()) + ' 23:59:59'
      }
      // console.log(this.$route.query.username)
      this.selectInput = this.allUserName || this.$route.query.username || ''
      // 深拷贝对象
      let data = this.copyObj(this.sendParmas)
      // 条件
      data.data[this.selectKey] = this.selectInput
    ...
    methods: {
      ...
      checkeDmDetail() {
        ...
        this.allDmVisible =true;
        this.betsCodeInfo({uid:this.getUid})
                .then(res=>{
                  this.allDmData = res.data;
                  const dom =  this.$refs.dmDOm,
                        tabHead = this.$refs.tabHead;
                  dom.addEventListener('scroll',()=>{
                    if(dom.scrollTop>60) {
                      tabHead.className = 'head-odd';
                    }else {
                      tabHead.setAttribute('class','head')
                    }
                  })
                })
      },
...
//      修改真实姓名
      ...
      commitCheck () {
        ...
        if (list.length > 0) {
          ...
          this.$message( this.$L.t('member.info161'))
        }
      },
      // 批量调整分组 提交选中
      openGrouping () {
      ...
      submitGrouping () {
        ...
        if (this.formData.groupId.toString() == '') {
          this.$message( this.$L.t('member.info163'))
          return false
        }
        this.setUsersGroup({
      ...
      commitAddUser () {
        ...
        if (!this.userForm.exclass_id) {
          this.$message(this.$L.t('member.info164'))
          return false
        }
        ...
        if (this.userForm.type == 1) {
          this.addUserAction({
            ...
            exclass_id: this.userForm.exclass_id,
      ...
      // 冻结/启动
      enableChange (item, enable) {
        let uid = item.uid;
        this.getEnable({uid, enable,Parmas:this.getHeadParmas()})
                // .then(() => {
                //   // 更新列表数据
                //   this.getUserLists(this.getHeadParmas())
                // })
      ...
      editInfoClick (item) {
        ...
        console.log(item.isFanDian)
      },
      updateDML() {
        this.getUpdateDml({uid:this.uid,cnum:this.editInfoVal.needBetsCode})
                .then(res=>{
                  this.editInfoVal.needBetsCode = res.needBetsCode;
      ...
      formCommit (type) {
        ...
        if (type == 'sdckVisible' || type == 'zsljVisible') {
          if (!this.formData.payPrice) {
            this.$message(this.$L.t('member.info125'))
            return false
          }
