<template>
    <div class="container">
      <table class="table" >
        ...
        <tr class="nav">
          <td colspan="10">
            {{$L.t('setup.info183')}}
            <input @click="addInfodata()" type="submit" :value=" $L.t('setup.info184') "></td>
        </tr>
        <tr class="head">
        ...
        <tr class="odd" v-for="item in setupStore.dataHelpList.data.data">
          <td>{{item.title}}</td>
        ...
        <tr v-if="getPageData.sum>0">
          ...
          <tr v-if="!setupStore.dataHelpList.data.data">
      ...
      <el-dialog
        ...
        <div v-if="editInfoVisible" class="form_epayindex">
          <div class="formBox_title">{{$L.t('setup.info185')}}</div>
          <ul>
            <li>
  ...
  <script>
    ...
    export default {
              ...
              toolbar: {
                ...
                handlers: {
                  'image': function (value) {
                    if (value) {
                      document.querySelector('#quill-layer-upload input').click()
                    } else {
      ...
      created (){
        this.getHelpList(this.sendParmas)
      },
      methods: {
        ...mapMutations([
        ]),
        ...
        submitVal(){
          if(this.visibleNum == 1){//新增
            this.addHelpList(this.addList).then(res=>{
              this.editInfoVisible = false
              this.getHelpList(this.sendParmas)
            })
          }else{//修改
            this.addList.id = this.editid
            this.updateHelpList(this.addList).then(res=>{
              this.editInfoVisible = false
              this.getHelpList(this.sendParmas)
            })
          }
      ...
      computed: {
        ...
        getPageData () {
          return {
            ...
            sum: this.setupStore.dataHelpList.data.count || 0
          }
        }
      },
    }
