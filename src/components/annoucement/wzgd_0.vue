<template>
    <div class="container">
      <table class="table" >
        ...
        <tr class="odd" v-for="item in annoucementStore.annoutList.data.data">
          <td>{{item.title}}</td>
          <td class="operation">{{item.content}}</td>
        ...
        <tr v-if="getPageData.sum>0">
          ...
          <tr v-if="!annoucementStore.annoutList.data.data">
      ...
      <el-dialog
        ...
        <div v-if="editInfoVisible" class="form_container">
          <div class="formBox_title">{{$L.t('annoucement.wjtz_3.zh17')}}</div>
            <ul>
              <li>
              ...
              <li>
                ...
                <div class="formLi_content">
                  <textarea rows="10" cols="70" name="content" id="content"  :placeholder="$L.t('annoucement.tcgg_1.importContent')" v-model="addList.content"></textarea>
                </div>
              </li>
              <li v-show="status">
                <div class="formLi_title">
  ...
  <script>
    ...
    export default {
      ...
      created (){
        this.getAnnouList(this.sendParmas)
      },
      mounted(){
      ...
      methods: {
        ...
        submitVal(){
          if(this.visibleNum == 1){//新增
            this.addAnnouList(this.addList).then(res=>{
              this.editInfoVisible = false
              this.getAnnouList(this.sendParmas)
            })
          }else{//修改
            this.addList.id = this.editid
            this.updataAnnouList(this.addList).then(res=>{
              this.editInfoVisible = false
              this.getAnnouList(this.sendParmas)
            })
          }
        ...
        commitModel(){
          this.deleteAnnouList({id:this.confirmId}).then(res=>{
      ...
      computed: {
        ...
        getPageData () {
          return {
            ...
            sum: this.annoucementStore.annoutList.data.count
          }
        }
      },
    }
